...
// Funciones
const mostPopulatedCountries = (arr) => {
  let sum = sumPopulatedCountries(arr);
  const tenCountries = [];
  tenCountries.push({ countrie: "World", population: sum });
  for (let key in arr) {
    let name = arr[key].name;
    let population = arr[key].population;
    tenCountries.push({ countrie: name, population: population });
  }
  tenCountries.sort((a, b) => {
    if (a.population < b.population) return 1;
    if (a.population > b.population) return -1;
  });

  tenCountries[3].countrie = "USA";
  tenCountries[9].countrie = "Russia";
  return tenCountries.slice(0, 10);
};

const sumPopulatedCountries = (arr) => {
  let sum = 0;
  for (let key in arr) {
    sum += arr[key].population;
  }
  return sum;
};

const mostLenguaguesSpoken = (arr) => {
  const lenguaguesCount = {};
  arr.forEach((element) => {
    let lenguage = element.languages[0];
    lenguaguesCount[lenguage] = (lenguaguesCount[lenguage] || 0) + 1;
  });
  let result = Object.entries(lenguaguesCount).map(([letter, count]) => ({
    letter,
    count,
  }));
  result.sort((a, b) => {
...
// Descomponiendo
const sum = sumPopulatedCountries(countries_data);
const valuesOne = mostPopulatedCountries(countries_data);
const labelOne = valuesOne.map((element) => element.countrie);
const dataOne = valuesOne.map((element) => element.population);

const valueTwo = mostLenguaguesSpoken(countries_data);
const labelTwo = valueTwo.map((element) => element.letter);
const dataTwo = valueTwo.map((element) => element.count);
let ctx = document.getElementById("stats");

// Subtitle
const subtitle = document.querySelector(".subtitle");
subtitle.innerHTML = "Currently, we have 250 countries";
...
const firstChar = () => {
  ...
  ctx.setAttribute("class", "graph-wrapper");
  ctx.setAttribute("id", "stats");
  graph.append(ctx);
...
population.addEventListener("click", (e) => {
  graphTitle.innerHTML = "10 Most Populatied Countries In The World ";
  graph.removeChild(ctx);
  firstChar();
});
